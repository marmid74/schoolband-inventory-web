    ...
    <v-card style="width: 100%;">
      ...
      >
        <template slot="items" slot-scope="row">
          <td>{{ row.item.nr }}</td>
          <td>{{ row.item.instrument.itemtype }}</td>
          <td>{{ row.item.instrument.brand }}</td>
          <td>{{ row.item.instrument.model }}</td>
          <td>{{ row.item.instrument.quality}}</td>
...
import UpdateBitems from './UpdateInstruments.vue'
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      {
        text: 'Brand', align: 'center', value: 'type'
      },
      {
        text: 'Model', align: 'center', value: 'model'
      },
      {
        text: 'Quality', align: 'center', value: 'status'
      },
      {
